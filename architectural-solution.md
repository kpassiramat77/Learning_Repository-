# Architectural Solution: Multi-Agent LLM System for Excel to Target JSON Conversion
Author: Manus AI  
Date: February 7, 2026

## 1. Introduction: The Core Challenge
Your current prototype for converting Excel-based actuarial models into a target JSON payload faces a significant limitation: any minor change in the Excel model necessitates a complete rebuild of the entire JSON payload. This process is inefficient, error-prone, and difficult to scale, especially as the complexity of the actuarial models grows. A change to a single formula should not require regenerating the entire structure.
...
These specialized agents work in parallel, each producing a small, self-contained fragment of the final JSON. An orchestrator then assembles these fragments into the complete, valid payload. This modularity is the key to solving your problem.
...
- **Factor Agent:** Receives `factors.csv`. Its sole job is to use its highly-tuned prompt to convert this simple key-value data into the specific JSON structure that the target system expects for defining factors. It outputs a `factor_fragment.json`.
- **Lookup Table Agent:** Receives `lookups.csv`. It is an expert at creating the target JSON for table lookups. It outputs a `lookup_fragment.json`.
- **Formula/Calculation Agent:** This is the most complex agent. It receives the `formulas.csv` data. Its prompt is engineered to be an expert in translating Excel formula syntax (like VLOOKUP, IF, SUMIFS) into the equivalent target calculation language within a JSON structure. It outputs a `calculation_fragment.json`.

### Step 4: Assembly and Validation
Once the specialized agents have completed their work, their outputs need to be combined.

1. **JSON Assembler:** This is a relatively simple component. It takes the various JSON fragments (`factor_fragment.json`, `lookup_fragment.json`, etc.) and intelligently merges them into a single, complete JSON document based on the required target payload structure.
2. **JSON Validator:** Before sending the payload to the target system, it's crucial to validate it. This component checks the assembled JSON against a known target JSON Schema (if one exists) or runs a series of checks to ensure all required fields are present and correctly formatted. This prevents sending invalid data to the API.

### Step 5: Final Output
The final, validated JSON payload is now ready and is sent to the target API endpoint for import.
...
7. The updated payload is validated and sent to the target API.
...
You are an expert JSON developer. Your sole responsibility is to convert CSV data representing a lookup table into the valid JSON format for a "Table Lookup" component in the target system. You will be given the table name and the table data in CSV format. Do not add any other components. Do not explain your work. Only output the raw JSON object.
...
You are an expert JSON developer specializing in calculation logic. Your task is to convert a single Excel formula into the corresponding JSON calculation block for the target system. You must accurately translate Excel functions like VLOOKUP, IF, and standard arithmetic operators into their target equivalents.

**Excel Formula:** `=IF(VLOOKUP(A5, AgeBanding, 2, FALSE) > 1.1, "High Risk", "Standard Risk")`

**Context:**
- `A5` refers to the "ApplicantAge" input variable.
- `AgeBanding` is a pre-defined table lookup component in the target system.
